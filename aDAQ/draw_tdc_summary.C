{
	...
	for(Int_t asic = 0; asic < 4; asic += 1) {
		for(Int_t channel = 0; channel < 64; channel+= 1) {
			...
			gSummary->SetPoint(nPoints, 64*asic + channel, sigma * 6.25E-9);			
			gSummary->SetPointError(nPoints, 0, sigmaError * 6.25E-9);
			
			hSummary->Fill(sigma * 6.25E-9);
			
			hCounts->Fill(64*asic +channel, hT->GetEntries());
	...
	for(Int_t asic = 0; asic < 4; asic += 1) {
		for(Int_t channel = 0; channel < 64; channel+= 1) {
			for (Int_t tac = 0; tac < 4; tac += 1) {
				...
				gLeakage->SetPoint(nPoints, 64*asic + channel + 0.25*tac, leakage);
				nPoints += 1;
