...
//	TVirtualFitter::SetDefaultFitter("Minuit2");
	...
	float timeGain = boost::lexical_cast<float>(argv[2]);
	char *tableFileName1 = argv[3];
	char *tableFileName2 = argv[4];
...
//				if((ftFine >= 0.90 * tiT.minY) && (ftFine <= 1.10 * tiT.maxY)) { 
//				if(qT > 0.9 && qT < 3.1) {
				if(isNormalT) {
					tiT.pControlT->Fill(fStep2, tT - fStep2);
					tiT.pControlE->Fill(tT - fStep2);
				}
				tacInfo[2*index + 0].pControlQ->Fill(fStep2, qT);
			}

			if((tiE.pControlT != NULL)) {
//				if ((feFine >= 0.90 * tiE.minY) && (feFine <= 1.10 * tiE.maxY)) { 
//				if (qE > 0.90 && qE < 3.1) {
				if(isNormalE) {
					tiE.pControlT->Fill(fStep2, tE - fStep2);
					tiE.pControlE->Fill(tE - fStep2);
				}
				tacInfo[2*index + 1].pControlQ->Fill(fStep2, qE);
			}
