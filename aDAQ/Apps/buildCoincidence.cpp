...
int main(int argc, char *argv[])
{
	...
	for(int step = 0; step < N; step++) {
		...
		EventSink<Coincidence> * writer= new EventWriter(lmData, gWindow, maxHitsRoot);
		if(useROOT==false) {
			delete writer;
			EventSink<Coincidence> * writer= new EventWriter(outListFile, gWindow, 1, acqAngle, ctrEstimate);
		}
	

		EventSink<RawPulse> * pipeSink = new CoincidenceFilter(Common::getCrystalMapFileName(), 1.25 * cWindow, minToT,
